...
Simulation* init_simulation(int num_locations, int num_couriers, const char* log_filename)
{
	...
	sim->couriers_list = init_couriers(num_couriers);
...
void handle_new_order(Simulation* sim, event* current_event)
{
	...
	customer* cust;
	cust = find_customer_by_id(sim->customer_queue,my_order->customerID);
	courier* found_courier = find_closest_courier(sim, cust->resX, cust->resY);
	if (found_courier != NULL)
	{
		found_courier->status = BUSY;
		assign_courier_to_order(my_order, found_courier->id);
		time = calculate_delivery_time(cust);
		pickup_time=sim->current_time + time;
		pickup_event = create_event(order_pickup, pickup_time, atoi(my_order->id), my_order);
		if (pickup_event == NULL)
		{
			fprintf(stderr, "no event created\n");
			return;
		}
		enqueue_event(&sim->event_queue, pickup_event);
	}
	else
	{
...
void handle_order_pickup(Simulation* sim, event* current_event)
{
	...
	if (my_cust != NULL)
	{
		travel_time = calculate_delivery_time(my_cust);
...
void handle_order_delivered(Simulation* sim, event* current_event)
{
	...
	if (my_customer != NULL && my_courier != NULL)
	{
		...
		update_customer_status(my_customer, RECEIVED);
		...
		if (my_courier->individual_deliveries_count >= 3)
		{
			printf("  -> [SHIFT END] Courier %s finished 3 deliveries and is going home.\n", my_courier->id);

			// מוחקים את השליח מהרשימה המקושרת!
			// שים לב שצריך לשמור את ראש הרשימה החדש שחוזר מהפונקציה
			sim->couriers_list = remove_single_courier(sim->couriers_list, my_courier->id);

			// אפשר גם לעדכן את הכמות הכללית של השליחים
			sim->total_couriers--;
		}
		else
		{
...
{
	order* my_order = (order*)current_event->data;
	double delay_time = 15.0;
	double new_delivery_time = 0.0;
	event* delayed_event = NULL;
	new_delivery_time = sim->current_time + delay_time;
...
void handle_order_canceled(Simulation* sim, event* current_event)
{
	...
	if (strcmp(my_order->courierID, "NONE") != 0)
	{
		my_courier = find_courier_by_id(sim->couriers_list, my_order->courierID);
...
void load_data_from_file(Simulation* sim, const char* filename)
{
	...
	while (fscanf(fin, "%s %s %s %s %lf", order_id, food, restaurant, cust_id, &creation_time) == 5)
	{
		...
		new_event= create_event(new_order, creation_time, atoi(order_id), n_order);
		if (new_event == NULL)
		{
			fprintf(stderr, "no event created\n");
			return;
...
courier* find_closest_courier(Simulation* sim, int resX, int resY)
{
	...
	while (curr != NULL)
	{
		...
		if (c->status == IDLE)
		{
			dx = abs(c->currentX - resX);
			dy = abs(c->currentY - resY);
			if (dx >= sim->total_locations) dx = sim->total_locations - 1;
			if (dy >= sim->total_locations) dy = sim->total_locations - 1;
			curr_distance = sim->distance_matrix[dx][dy];
